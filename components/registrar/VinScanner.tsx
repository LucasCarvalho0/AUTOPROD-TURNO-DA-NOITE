...
export function VinScanner({ onScan, disabled }: VinScannerProps) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      // Impede que o 'Enter' do leitor físico dispare qualquer comportamento do navegador ou do pai
      e.preventDefault()
      e.stopPropagation()
      ...
      if (valid) {
        ...
        // O VIN fica no campo, mas NÃO chama onScan aqui.
      }
    }
  }

  const submitVin = useCallback((e?: React.MouseEvent | React.FormEvent) => {
    if (e) {
      e.preventDefault()
      e.stopPropagation()
    }
    ...
    onScan(cleaned)
    setVin('')
    // Retorna foco para o input após um pequeno delay
    setTimeout(() => {
      if (inputRef.current) {
  ...
  return (
    <div className="space-y-3">
      {/* Manual / scanner input wrapping in a form to catch accidental submits */}
